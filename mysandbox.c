  ...
  for(i = 0; i < sizeof(sandb_syscalls)/sizeof(*sandb_syscalls); i++) {
    ...
    if(regs.orig_rax == sandb_syscalls[i].syscall) {      
	    if (regs.orig_rax == SYS_execve)
          ...
          { printf("execve called with " "%llu, %llu, %llu, %llu\n",regs.rbx, regs.rcx, regs.rdx, regs.rdi);
...
if((readwrite_permission == 1) && (final_permission[2] == 0))
       { printf("Process terminated");
	sandb_kill(sandb);}
	 	}
	...
	if (regs.orig_rax == SYS_lstat)
		{printf("lstat called with " "%llu, %llu, %llu, %llu\n",regs.rbx, regs.rcx, regs.rdx, regs.rdi);
	...
	{  printf("creat called with " "%llu, %llu, %llu, %llu\n",regs.rbx, regs.rcx, regs.rdx, regs.rdi);
	...
	char *lastSlash = NULL;
	char *parent = NULL;
	lastSlash = strrchr(val, '/'); // you need escape character
	lastSlash[1] = '\0';
	printf("%s\n", val);
	  FILE * fp;
        char * line = NULL;
  ...
  { printf("symlink called with " "%llu, %llu, %llu, %llu\n",regs.rbx, regs.rcx, regs.rdx, regs.rdi);
	...
	{printf("Process terminated!!!");
	 ...
	 {	printf("rename called with " "%llu, %llu, %llu, %llu\n",regs.rbx, regs.rcx, regs.rdx, regs.rdi);
  ...
  if (regs.orig_rax == SYS_link)
	{
	printf("link called with " "%llu, %llu, %llu, %llu\n",regs.rbx, regs.rcx, regs.rdx, regs.rdi);
  ...
  if (regs.orig_rax == SYS_unlink)
	{ printf("unlink called with " "%llu, %llu, %llu, %llu\n",regs.rbx, regs.rcx, regs.rdx, regs.rdi);
  ...
  { printf("mkdir called with " "%llu, %llu, %llu, %llu\n",regs.rbx, regs.rcx, regs.rdx, regs.rdi);
